//
//  Section23_iOS7_3ViewController.m
//  YX
//
...
#import "Section23_iOS7_3ViewController.h"
//
#include <stdio.h>
#include <string.h>
#include <sys/types.h>
...
@implementation Section23_iOS7_3ViewController
...
- (void)HTTPDownloadContentsFromHostName:(NSString *)hostName
    ...
    //创建socket
    dispatch_fd_t socket = [self connectToHostName:hostName port:port];//创建和服务器的socket的链接
    //通过上面的socket创建一个通道, 通道读取方式，文件描述符（句柄），队列，清理块
    ...
    //先把数据写入socket通道，再读出写入文件通道
    dispatch_io_write(serverChannel, 0, requestData, queue, ^(bool serverWriteDone, dispatch_data_t serverWriteData, int serverWriteError) {
        NSAssert(!serverWriteError, @"Server write error:%d", serverWriteError);
        if (serverWriteDone) {
...
- (void)readFromChannel:(dispatch_io_t)readChannel writeToChannel:(dispatch_io_t)writeChannel queue:(dispatch_queue_t)queue
{
    ...
    dispatch_io_read(readChannel, 0, SIZE_MAX, queue, ^(bool serverReadDone, dispatch_data_t serverReadData, int serverReadError) {
        NSAssert(!serverReadError, @"Server read error:%d", serverReadError);
        if (serverReadDone) {
            //关闭所有通道
            ...
            if (!headerData) {
                ...
                [self writeToChannel:writeChannel data:serverReadData queue:queue];
            }
        }
    });
}
...
{
    dispatch_io_write(channel, 0, writeData, queue, ^(bool done, dispatch_data_t remainingData, int error) {
        ...
        NSLog(@"Wrote %zu bytes", dispatch_data_get_size(writeData) - unwrittenDataLength);
    });
}
